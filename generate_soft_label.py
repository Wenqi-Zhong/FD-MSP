...
def get_arguments():
    ...
    parser.add_argument('--output_folder', type=str, default='./datasets/polyp_soft_labels')
...
def validate(model_B2, model_B, head, classifier, seg_loss, gpu, logger, testloader, output_folder):
    ...
    with torch.no_grad():
        for i, batch in enumerate(testloader):
            ...
            for b in range(min(output_np.shape[0], len(img_paths))):
...
if __name__ == '__main__':
    ...
    if args.dist:
        ...
        # 不使用分布式模式时，创建一个默认的URL
        dist_url = "tcp://127.0.0.1:12345"
        main_worker(args.gpus[0], args.world_size, dist_url)
