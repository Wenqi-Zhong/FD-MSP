...
class Kvasir_val_loader(BaseDataset):
    """Kvasir-SEG验证数据集加载器
    ...
    def __init__(self, opt, logger, augmentations = None, split='val'):
        ...
        # 为简化起见，我们将前20%的图像用作验证集
        num_total = len(self.files)
        val_ratio = 0.2
        val_count = int(num_total * val_ratio)
        
        if split == 'val':
            self.files = self.files[:val_count]
        else:  # 如果不是验证集，则使用剩余部分（用于其他用途）
            self.files = self.files[val_count:]
    ...
    def __getitem__(self, index):
        ...
        return {'img': img, 'label': lbl, 'img_path': img_path}
