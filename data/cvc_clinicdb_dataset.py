...
class CVC_ClinicDB_loader(BaseDataset):
    """CVC-ClinicDB数据集加载器
    ...
    def __init__(self, opt, logger, augmentations = None, split='train'):
        ...
        # 对于CVC-ClinicDB，我们可以根据是否指定了pseudo_labels_folder来决定使用原始mask还是伪标签
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train' and hasattr(self.opt, "soft_labels_folder") and self.opt.soft_labels_folder:
            ...
            if os.path.exists(lpsoft_path):
                ...
                # 软标签文件包含logits，需要转换为概率分布
                if lpsoft.ndim == 2:  # 如果是 [H, W] 的logits
                    # 对于二分类，创建背景和前景的logits
                    background_logits = -lpsoft  # 背景logits为负值
                    foreground_logits = lpsoft   # 前景logits为正值
                    lpsoft = np.stack([background_logits, foreground_logits], axis=0)  # [2, H, W]
                    
                    # 应用softmax转换为概率
                    lpsoft = np.exp(lpsoft) / np.sum(np.exp(lpsoft), axis=0, keepdims=True)
                elif lpsoft.shape[0] == 1:  # 如果是 [1, H, W] 的logits
                    # 对于二分类，创建背景和前景的logits
                    background_logits = -lpsoft[0]  # 背景logits为负值
                    foreground_logits = lpsoft[0]   # 前景logits为正值
                    lpsoft = np.stack([background_logits, foreground_logits], axis=0)  # [2, H, W]
