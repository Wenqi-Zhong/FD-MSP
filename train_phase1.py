...
def main_worker(gpu, world_size, dist_url):
    ...
    if args.model == 'DeepLab':
        ...
        if args.resume:
            if args.resume.startswith('http'):
                resume_weight = torch.hub.load_state_dict_from_url(args.resume, progress=True, map_location='cpu')
            else:
                resume_weight = torch.load(args.resume, map_location='cpu')
            print("args.resume: ", args.resume)
            
            # 由于sourceonly.pth的模型结构与代码期望的不匹配，我们将使用标准的ResNet101模型
            if gpu == 0:
                logger.info("使用PyTorch预训练的ResNet101模型代替sourceonly.pth")
        ...
        if args.layer == 0:
            model_B1 = nn.Sequential(model[0], model[1], model[2], model[3])
        elif args.layer == 1:
            model_B1 = nn.Sequential(model[0], model[1], model[2], model[3], model[4])
        elif args.layer == 2:
            model_B1 = nn.Sequential(model[0], model[1], model[2], model[3], model[4], model[5])
        ...
        aux_classifier = ASPP_Classifier_Gen(2048, [3, 6, 9, 12], [3, 6, 9, 12], args.num_classes, hidden_dim=args.hidden_dim)
    ...
    # 添加类别权重，提高息肉类的权重（Class_1）
    class_weights = torch.tensor([1.0, 10.0]).cuda()  # 背景类权重=1.0，息肉类权重=2.5
    seg_loss = torch.nn.CrossEntropyLoss(weight=class_weights, ignore_index=args.ignore_label)
    if gpu == 0:
        logger.info("使用类别权重: {}".format(class_weights))
...
def validate(model_B2, model_B, head, classifier, seg_loss, gpu, logger, testloader):
    ...
    dice_meter = AverageMeter()  # 添加Dice度量
    ...
    with torch.no_grad():
        for i, batch in enumerate(testloader):
            ...
            intersection, union, _, dice = intersectionAndUnionGPU(output, labels, args.num_classes, args.ignore_label)
            dist.all_reduce(intersection), dist.all_reduce(union), dist.all_reduce(dice)
            intersection, union, dice = intersection.cpu().numpy(), union.cpu().numpy(), dice.cpu().numpy()
    ...
    dice_class = dice_meter.sum / (dice_meter.count + 1e-10)
    mdice = np.mean(dice_class)
    if gpu == 0:
        logger.info("Val result: mIoU = {:.3f}, mDice = {:.3f}".format(miou, mdice))
        for i in range(args.num_classes):
            logger.info("Class_{} Result: iou = {:.3f}, dice = {:.3f}".format(i, iou_class[i], dice_class[i]))
